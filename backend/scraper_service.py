...
class DiarioScraper:
    def __init__(self, debug: bool = False):
        ...
        self.partial_results_file = base_dir / "partial_results.json"
    ...
    def _save_partial_results(self, results):
        ...
        try:
            ...
            self.partial_results_file.write_text(
                json.dumps(temp_results, ensure_ascii=False, indent=2),
                encoding="utf-8",
            )
        except Exception as exc:
            logger.error("Erro ao salvar resultados parciais: %s", exc)
    ...
    def _extract_modality(self, text, data):
        if data.get('modality') in ["-", "", None]:
            m_mod = re.search(
                r'(PREGÃO ELETRÔNICO|PREGÃO|CONCORRÊNCIA|TOMADA DE PREÇOS|CONVITE|LEILÃO|DIÁLOGO COMPETITIVO|INEXIGIBILIDADE|DISPENSA)',
                text,
                re.IGNORECASE,
            )
            if m_mod:
    ...
    def _extract_contract_info(self, text, data):
        if data.get('num_contrato') in ["-", "", None]:
            m_id = re.search(
                r'(?:Pregão(?: Eletrônico)?|Contrato|Licitação|Carta Convite|Nota de Empenho|Termo de Fomento|Termo de Colaboração|Acordo de Coopera[çc][ãa]o|Termo de Doação|Termo de Comodato)\s*(?:nº|n°)?\s*([\d\.]+(?:/[\d]{2,4})?)',
                text,
                re.IGNORECASE,
            )
            if m_id:
        ...
        if m_adit:
            ...
            m_pai = re.search(
                r'ao (?:Termo de )?(?:Contrato|Termo de Colaboração|Termo de Fomento|Ajuste)\s*(?:nº|n°)?\s*([\d\.]+(?:/[\d]{2,4})?)',
                text,
                re.IGNORECASE,
            )
            if m_pai:
    ...
    def _extract_dates(self, text, data):
        ...
        if validade_inicio:
            validade_inicio = normalize_date(validade_inicio)
        ...
        if not found_vig and validade_inicio and data.get('prazo'):
            try:
                ...
                if not tipo:
                    prazo_text = data['prazo'].lower()
                    if 'mês' in prazo_text or 'meses' in prazo_text:
                        tipo = 'mês'
                    elif 'ano' in prazo_text:
                        tipo = 'ano'
                    elif 'dia' in prazo_text:
                        tipo = 'dia'
                ...
                logger.warning("Falha ao calcular vigência por prazo: %s", exc)
            except Exception as exc:
                logger.error("Erro inesperado ao calcular vigência por prazo: %s", exc)
    ...
    def _classify_document(self, text, data):
        ...
        if modality == 'DISPENSA' or "DISPENSA" in modality:
            data['tipo_doc'] = 'PEDIDO_COMPRA'
        elif re.search(r'(?:Formalização|Termo|Extrato) d[oa] Contrato', text, re.IGNORECASE) or re.search(r'Contrato\s*(?:nº|n°)\s*[\d]+', text, re.IGNORECASE):
    ...
    def _apply_shielding(self, data):
        ...
        for campo, msg in criticos:
            ...
            if not valor or valor in ["-", "", None]:
                logger.warning("[BLINDAGEM] %s", msg)
    ...
    async def enrich_with_ai(self, details, item_id, enabled=True):
        ...
        try:
            from ai_extractor import extract_with_gemini, is_ai_enabled

            if not is_ai_enabled():
                return

            logger.info("Enriquecendo documento %s com IA...", item_id)
            ai_data = await asyncio.wait_for(extract_with_gemini(details['sintese']), timeout=30.0)

            if ai_data:
                logger.debug("IA retornou dados para %s", item_id)
                mapeamento = {
            ...
            logger.error("Falha na IA para doc %s: %s", item_id, exc)

    def extract_object(self, text):
        if not text:
            return "VERIFICAR NA ÍNTEGRA"
        txt = re.sub(r'\s+', ' ', text)
        ...
        if "PRORROG" in txt.upper() or "ADITAMENTO" in txt.upper():
            ...
            if m_prorrog:
                ...
                return txt[start:end + 20].strip('.,; ')

        match_obj = re.search(
            r'(?:OBJETO da licitação|OBJETO|ASSUNTO):?\s*(.*?)(?=\s*(?:JULGAMENTO|REGIME|MODALIDADE|MODO|Valor|Prazo|Local|Data|Edital|Sessão|II\s?-|II\.|\.|$))',
            txt,
            re.IGNORECASE,
        )
        if match_obj:
    ...
    async def scrape(self, start_date: str | datetime, end_date: str | datetime, terms: list, status_callback=None, use_ai=True):
        ...
        try:
            d1 = datetime.strptime(start_date, "%d/%m/%Y") if isinstance(start_date, str) else start_date
            d2 = datetime.strptime(end_date, "%d/%m/%Y") if isinstance(end_date, str) else end_date

            if d1 > d2:
                d1, d2 = d2, d1

            date_list = [(d1 + timedelta(days=i)).strftime("%d/%m/%Y") for i in range((d2 - d1).days + 1)]
            logger.info("Iniciando navegador (debug=%s)...", self.debug)
            ...
            async with async_playwright() as p:
                ...
                for channel in (None, "chrome", "msedge"):
                    try:
                        if channel:
                            browser = await p.chromium.launch(channel=channel, **launch_options)
                        else:
                            browser = await p.chromium.launch(**launch_options)
                        break
                    except Exception as exc:
                        logger.debug("Falha ao iniciar navegador com canal %s: %s", channel or "chromium", exc)
                        continue

                if not browser:
                    raise RuntimeError("Falha crítica ao iniciar navegador.")
                ...
                for day_idx, current_date in enumerate(date_list):
                    ...
                    try:
                        ...
                        logger.error("Falha ao buscar %s", current_date)
                        continue
                    ...
                    async def fetch_and_extract(item):
                        ...
                        async with sem:
                            ...
                            try:
                                ...
                                if not obj_text or len(obj_text) <= 5:
                                ...
                                if status_callback:
                                    await status_callback(f"Extraindo item {day_processed_count} de {total_items} ({current_date})")
                                return res
                            except Exception as exc:
                                logger.error("Erro no item %s: %s", item['doc_id'], exc)
                                day_processed_count += 1
                                return None

                    day_results = await asyncio.gather(*[fetch_and_extract(it) for it in links_to_visit])
            ...
            if status_callback:
                await status_callback(finish_msg)
            return results

        except Exception as exc:
            logger.error("Erro fatal no scraping: %s", exc)
            raise
        finally:
            self.is_running = False
